...
公司业务需要播放 h265 格式的视频，但是目前浏览器不支持 h265 格式，所以需要使用 mp4box 播放 h265 格式的视频。

mp4box.js 是一个开源的库，用于解析和播放 MP4 文件。它提供了一组 API，用于解析和播放 MP4 文件，以及一些工具，用于创建和编辑 MP4 文件。

前端使用 react 框架、后端使用 rtsp 协议播放视频,后端生成的后缀路径为后缀为.live.mp4![alt text](image.png)

安装 mp4box.js

```bash
npm install mp4box.js
```
...
class StreamH265Player {
  ...
  putPacket(arr) {
    ...
    if (
      this.streamLiveDom.duration >= this.BUFFERING_SECOND &&
      ...
      if (df > this.BUFFERING_SECOND_SEEK) {
        this.streamLiveDom.currentTime =
          this.streamLiveDom.duration - this.BUFFERING_SECOND_SEEK_DISTANCE;
      }
    }
  ...
  _raisePlayerSocket() {
    ...
    this.socketInstance.onerror = function (e) {
      console.log(e, "error");
    };

    this.socketInstance.onopen = function (e) {
      console.log(e, "open");
    };

    this.socketInstance.onclose = function (e) {
      console.log(e, "close");
    };
  }
  ...
  startToPlay(url, dom) {
    ...
    this.mediaSourceOpenHandler = this._bindFunction(
      this,
      this.onMediaSourceOpen
    );
    this.mediaSource.addEventListener(
      "sourceopen",
      this.mediaSourceOpenHandler
    );
